...
import { createUser , findUserByemail,createOAuthUser,FindOauthUser, createOauthMethod, createUserSession, verifyCredUser,findUserSession,deleteExpiredUserSessions } from '../../Models/userModel';
...
export const verifyGoogleProfileAndLogin = async (profile: any) => {
  try {
    ...
    if (userResult && userResult.rows?.[0]) {
      ...
      if (user.auth_type === 'Credentials') {
        throw new Error('Please sign in using Credentials, you cannot use OAuth.');
      }
...
export const handleGoogleCallback = async (req: Request, res: Response) => {
  ...
  if (!jwtToken) {
    return res.status(500).json({ error: "JWT not generated" });
  }

  // FRONTEND REDIRECT — SEND JWT TO /oauth-success
  const redirectURL = `http://localhost:5173/oauth-success?token=${jwtToken}`;
